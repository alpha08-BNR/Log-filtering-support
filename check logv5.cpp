...
struct Rule {
    std::string category;
    std::vector<std::string> fastKeywords; // Lowercase keywords for safe pre-filtering
    std::regex pattern;                    
};
...
struct ProcessContext {
    ...
    std::vector<long long> lineNumbers; // Store line numbers instead of full strings to optimize RAM
};
...
int main() {
    ...
    // Rules with ALL-LOWERCASE fastKeywords for accurate pre-filtering
    std::vector<Rule> rules = {
        ...
        },
        {
            ...
            {"connect", "wsaconnect", "internetconnect", "httpopenrequest", ":"}, // Added ':' to match IP:Port formats
    ...
    while (std::getline(inFile, line)) {
        ...
        // Convert current line to lower-case ONCE for fast case-insensitive pre-filtering
            ...
            if (isPreFilterMatched) {
                if (std::regex_search(line, rule.pattern)) {
                    if (!currentIdentity.empty()) {
                        ...
                        outFile << "[Standalone Alert - Line " << lineNumber << "] -> " << rule.category << "\n";
